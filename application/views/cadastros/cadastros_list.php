		...
		<div class="card">
			<div class="card-body">
				<div class="card-content table-responsive">
					<?= anchor(site_url('cadastros/create'),'<i class="fa fa-plus"></i> '.$this->lang->line('app_create'), 'class="btn btn-success"'); ?>
						...
						<form id="form_delete" method="post">
							<table id="table" class="table table-bordered" style="margin-bottom: 10px; width:100%">
								...
								<thead>
									<tr>
										<th>
										<?php if ($this->permission->check($this->session->userdata('permissao'), 'dCadastro')) { ?>
...
<script type="text/javascript">  
	$(document).ready(function () {
		...
		var datatable = $('#table').DataTable({
			...
			"order": [],
			"ajax": {
				url: "<?= site_url('cadastros/datatable'); ?>",
				type: "POST"
			},
			"columnDefs": [
				{
		...
		$('#form_delete').submit(function (event) {
			...
			function () {
                ...
                $.ajax({
					url: '<?= site_url('cadastros/delete_many'); ?>',
                    type: 'POST',
                    dataType: 'json',
                    data: data,
				})
