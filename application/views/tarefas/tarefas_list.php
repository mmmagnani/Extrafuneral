        ...
        <div class="card">
            <div class="card-body">
                <div class="card-content table-responsive">
                    <?= anchor(site_url('tarefas/create'),'<i class="fa fa-plus"></i> '.$this->lang->line('app_create'), 'class="btn btn-success"'); ?>
                            ...
                            <table id="table" class="table table-bordered table-striped" style="margin-bottom: 10px; width:100%">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th><?= ucfirst($this->lang->line('task_spo')) ?></th>
                                        <th>
                                            <?= ucfirst($this->lang->line('task_title')) ?>
                                        </th>
                                        <th style="text-align:center">
                                            <?= ucfirst($this->lang->line('task_estimated_value')) ?>
                                        </th>
                                        <th style="text-align:center">
                                            <?= ucfirst($this->lang->line('task_autorized_value')) ?>
                                        </th>
                                        <th style="text-align:center">
                                            <?= ucfirst($this->lang->line('task_executed_value')) ?>
                                        </th>
                                        <th width="100%" style="text-align:center; min-width:160px">
...
<script type="text/javascript">  
    $(document).ready(function () {
        ...
        var datatable = $('#table').DataTable({
            ...
            "order": [],
            "ajax": {
                url: "<?= site_url('tarefas/datatable'); ?>",
                type: "POST"
            },
            "columnDefs": [
                {
        ...
        $('#form_delete').submit(function (event) {
			...
			function () {
                ...
                $.ajax({
					url: '<?= site_url('tarefas/delete_many'); ?>',
                    type: 'POST',
                    dataType: 'json',
                    data: data,
				})
